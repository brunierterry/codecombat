(function (root, factory) {
  ...
  function frogDragonBossMission () {
    ...
    return {
      ...
      prePracticeId: SOURCE_CONCEPT_ID,
      title: 'カエルと守りのドラゴン',
      concept: 'if / else、看板、変身を組み合わせる総合ボス',
      story: 'ドラゴンの真上にゴールがあります。看板が示す側のスイレンを渡ってレバーへ行き、炎をさえぎる像を出さないと、ゴールへ近づいたときに焼かれてしまいます。',
      instructions: [
        '同じコードで2つのフィールドをクリアします。看板を読み、`left` なら左、それ以外なら右のスイレンの道へ進みましょう。',
        '`const goLeft = side === "left";` の結果を `if / else` で使えば、2つのフィールドを同じコードで進めます。',
        '川はカエルの姿で渡ります。ただしレバーを動かせるのは人の姿のヒーローだけです。',
        'レバーを動かすとドラゴンのすぐ上に像が現れ、上向きの炎をさえぎります。ドラゴンは倒れません。',
        '像はどの方向から来るドラゴンの炎も止めます。像の向こう側にいれば、炎はそこから先へ進みません。',
        'ゴールの左右にも像があるので、最後はドラゴンの正面を通り、下からゴールへ入らなければなりません。',
      ],
      api: [
        'hero.readSign()',
        'hero.transform("frog")',
      ...
      starterCode: [
        ...
        '// カエルに変身して、4マスの川を渡ろう',
        '',
        '// 向こう岸に着いたら人の姿に戻り、レバーを動かそう',
        '',
        '// 像で炎を止めたら、中央へ戻って上のゴールへ進もう',
      ].join('\n'),
      hints: [
        '`goLeft` は、看板が `left` のときだけ true になります。true なら左、false なら右へ進みましょう。',
        '左なら左へ3マス、右なら右へ3マス進み、カエルに変身して上へ5マス進むと向こう岸です。',
        'レバーはカエルの姿では動きません。向こう岸で `hero.transform("hero")` を使って人の姿に戻してから、レバーへ進みましょう。',
        'レバーで中央の像を出したあと、その像より上の列を中央まで進み、最後に真上のゴールへ入ります。',
      ],
      solution,
      variants: [
        {
          ...
          boss: {
            ...
            pillar: { x: 4, y: 3 },
            lever: { x: 3, y: 2 },
            protectiveStatue: { x: 6, y: 2 },
            attackRange: 3,
            resolution: 'protective-statue',
          },
        },
        {
          map: [
      ...
      bossEncounter: true,
      bossResolution: 'protective-statue',
      victoryConditions: [
        { id: 'max-moves', label: '移動：最大 14 回' },
      ],
      requirements: {
        state: { goal: true, minGems: 1, maxMoves: 14, noDragonFire: true, protectiveStatueRaised: true },
        syntax: [
