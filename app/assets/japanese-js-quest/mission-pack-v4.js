(function (root, factory) {
  ...
  function frogDragonBossMission () {
    const solution = [
      ...
      'hero.transform("frog");',
      '',
      'if (hero.canMove("up") && hero.look("up") === "lily") {',
      '  hero.move("up");',
      '  hero.move("up");',
      '  hero.move("up");',
      '  hero.move("up");',
    ...
    return {
      ...
      instructions: [
        ...
        '川はカエルの姿で渡ります。渡る前に `hero.canMove("up")` と `hero.look("up")` を `&&` で確認してください。',
        '向こう岸では人の姿に戻り、柱の外側からドラゴンの横を通ってレバーを踏みます。レバーを踏むとドラゴンを封印できます。',
        '最後に宝石を取ってゴールへ進みます。二つのフィールドで柱とレバーの位置が左右反対です。',
      ],
      api: [
        'hero.readSign()',
        'hero.canMove(direction)',
        'hero.look(direction)',
        'hero.transform("frog")',
        'hero.transform("hero")',
        'hero.move(direction)',
      ...
      starterCode: [
        ...
        '}',
        '',
        '// 人の姿に戻って、柱の外側からレバーとゴールへ進もう',
      ].join('\n'),
      hints: [
        '`goLeft` は、看板が `left` または `west` のとき true になります。それ以外は else 側です。',
        '左なら左へ3マス、右なら右へ3マス進んでから、カエルに変身します。スイレンは上へ4マスです。',
      ...
      requirements: {
        ...
        syntax: [
          ...
          syntax('comparison', '=== で看板とスイレンを調べましょう。'),
          syntax('logicalOr', '|| で left と west をまとめましょう。'),
          syntax('logicalAnd', '&& で渡れることとスイレンであることを両方確認しましょう。'),
          syntax('readSign', 'hero.readSign() で看板を読みましょう。'),
          syntax('canMove', 'hero.canMove(...) で進めるか確認しましょう。'),
          syntax('look', 'hero.look(...) でスイレンを確認しましょう。'),
